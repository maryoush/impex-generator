...
public class GenerateLocalizedImpexesBeanUnitTest
{
	...
	private final boolean sourceRelative =  false;

	private final String target = "bar";
	private final boolean targetRelative =  false;
	...
	@Spy
	private final Map<String, List<String>> templateFiles = new LinkedHashMap<String, List<String>>();

	@Spy
	@InjectMocks
	private final DefaultLocalizedImpexesGenerator bean = new DefaultLocalizedImpexesGenerator();

	private InOrder orderVerify;

	@Before
	public void prepare()
	{
		MockitoAnnotations.initMocks(this);
		bean.setSourcePath(source);
		bean.setSourcePathRelative(sourceRelative);
		bean.setTargetPath(target);
		bean.setTargetPathRelative(targetRelative);

		BDDMockito.given(localeResolver.getAllLocales()).willReturn(Arrays.asList(Locale.FRENCH));
		Mockito.doReturn(velocityExecutor).when(bean).getVelocityExecutor();
	...
	public void testOrderCall()
	{
		...
		BDDMockito.given(
				scriptUrlContextFactory.create(Mockito.eq(Locale.FRENCH), Mockito.any(File.class), Mockito.eq(source), Mockito.eq(sourceRelative),
						 Mockito.eq(target), Mockito.eq(targetRelative))).willReturn(urlCtx1, urlCtx2, urlCtx3, urlCtx4);
		...
		orderVerify = new InOrderImpl(Arrays.asList(localeResolver, templateFiles, scriptUrlContextFactory,
		...
		orderVerify.verify(scriptUrlContextFactory).create(Mockito.eq(Locale.FRENCH),
				Mockito.argThat(new FileMatcher("one", "ten")),  Mockito.eq(source), Mockito.eq(sourceRelative),  Mockito.eq(target), Mockito.eq(targetRelative));
		...
		orderVerify.verify(scriptUrlContextFactory).create(Mockito.eq(Locale.FRENCH),
				Mockito.argThat(new FileMatcher("one", "eleven")),  Mockito.eq(source), Mockito.eq(sourceRelative),  Mockito.eq(target), Mockito.eq(targetRelative));
		...
		orderVerify.verify(scriptUrlContextFactory).create(Mockito.eq(Locale.FRENCH),
				Mockito.argThat(new FileMatcher("two", "twenty")),  Mockito.eq(source), Mockito.eq(sourceRelative),  Mockito.eq(target), Mockito.eq(targetRelative));
		...
		orderVerify.verify(scriptUrlContextFactory).create(Mockito.eq(Locale.FRENCH),
				Mockito.argThat(new FileMatcher("two", "twentyone")),  Mockito.eq(source), Mockito.eq(sourceRelative),  Mockito.eq(target), Mockito.eq(targetRelative));
