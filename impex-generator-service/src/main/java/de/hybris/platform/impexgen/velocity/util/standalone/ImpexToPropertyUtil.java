...
public class ImpexToPropertyUtil
{
	...
	static protected class PropertyProcessor
	{
		...
		protected class Header
		{
			...
			private List<String> columns = null;//new ArrayList<String>(10);
		...
		public PropertyProcessor(final String path) throws IOException
		{
			...
			{
				final List<String> lines = FileUtils.readLines(impexFile, "UTF-8");
				process(lines);
			}
			else
			{
		...
		{
			//read line
			final String[] tokens = single.split(";");//new StringTokenizer(single, ";", true);single.split(";").
			String key = null;
			int startedAt = 0;
			...
			for (int i = 1; i < tokens.length; i++)
			{
				...
				else
				{
					...
					final String nextToken = tokens[i]; //tokens.nextToken();
